...
# Game variables
selected_section = None  # No section is selected at the start
sections = ["A", "B", "C", "D", "E", "F", "G", "H"]
gesture_state = "neutral"  # Tracks the current gesture state
stop_processing = False  # Controls video loop termination
...
def process_gesture(x_diff, y_diff):
    global selected_section, gesture_state

    if x_diff < -0.1 and gesture_state != "left":  # Move left
        print("Detected Gesture: Moving Left")
        gesture_state = "left"
        selected_section = (selected_section - 1) % len(sections) if selected_section is not None else 0
        update_interface()
    elif x_diff > 0.1 and gesture_state != "right":  # Move right
        print("Detected Gesture: Moving Right")
        gesture_state = "right"
        selected_section = (selected_section + 1) % len(sections) if selected_section is not None else 0
        update_interface()
    elif y_diff < -0.1 and gesture_state != "up":  # Select current section
        print(f"Detected Gesture: Selecting Section {sections[selected_section]}")
        gesture_state = "up"
        if selected_section is not None:
            on_selection(sections[selected_section])
    elif abs(x_diff) < 0.1 and abs(y_diff) < 0.1:  # Neutral gesture
